    ...
    public class SocketClient
    {
        ...
        private static void UpdatePlayInfo(string[] payload)
        {
            ...
            foreach (string playerName in playerNames)
            {
                ...
                {
                    players.Add(new PlayerTank { Name = playerName, Position = new PointF(0, 0) });
                }
            }
            ...
            for (int i = 1; i < payload.Length - 1; i += 2)
            {
                ...
                if (lobby == null)
                {
                    Lobby newLobby = new Lobby()
                    {
                        ...
                        Host = new PlayerTank { Name = payload[i + 1] },
                        PlayersName = new List<string> { payload[i + 1] },
                        Players = new List<PlayerTank>
                        {
                            new PlayerTank() {Name = payload[i + 1]}
                        }
                    };
        ...
        private static void HandleDisconnect(string playerName)
        {
            if (joinedLobby != null && joinedLobby.PlayersName.Contains(playerName))
            {
                ...
                if (joinedLobby.HostName == playerName)
                {
                    ...
                    joinedLobby.Host = new PlayerTank { Name = joinedLobby.PlayersName[0] };
                }
            }
        }
            ...
            if (lobby != null)
            {
                ...
                for (int i = 0; i < playerCount; i++)
                {
                    if (!lobby.PlayersName.Contains(playerList[i]))
                    {
                        ...
                        lobby.Players.Add(new PlayerTank()
                        {
                            Name = playerList[i],
        ...
        {
            string message = $"UPDATE_POSITION;{playerName};{direction};{X.ToString()};{Y.ToString()}";
            SendData(message);
        }
        ...
        private static void HandlePlayerPosition(string[] payload)
        {
            ...
            if (playerToUpdate != null)
            {
                ...
                playerToUpdate.DirectionTank = (Direction)Enum.Parse(typeof(Direction), direction);
